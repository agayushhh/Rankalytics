...
def plot_graph(request):
    ...
    file_path = os.path.join(settings.BASE_DIR, 'home', 'data', 'combined2.csv')
...
def branch_college_popularity(request):
    if request.method == 'POST':
        ...
        rank_adv = int(request.POST.get('rank_adv'))
        ...
        college_data = df[(df['Academic Program Name'].str.contains(branch, case=False)) & 
                          (df['Seat Type'] == seatty) & 
        ...
        iit_data = college_data[(((college_data['Opening Rank'] <= rank_adv) & (college_data['Closing Rank'] >= rank_adv)) | ((college_data['Opening Rank'] >= rank_adv))) & ((college_data['Institute'].str.contains('Indian Institute  of Technology')) | (college_data['Institute'].str.contains('Indian Institute of Technology')))]
        non_iit_data = college_data[(((college_data['Opening Rank'] <= rank_mains) & (college_data['Closing Rank'] >= rank_mains)) | ((college_data['Opening Rank'] >= rank_mains))) & (~college_data['Institute'].str.contains('Indian Institute of Technology'))]
    ...
    paper_bgcolor='rgba(0, 0, 0, 0)',  # Set paper background color to transparent
    font=dict(family="Courier New, monospace", size=14, color="#FFFFFF"),  # Change the font
    title=dict(text=f'Average Rank of Top 10 Different Colleges offering {branch}', font=dict(size=25, color='#FFFFFF', family='Courier New, monospace', weight='bold')),  # Bold and larger title
    legend=dict(font=dict(size=10, color='#FFFFFF')),  # Smaller legend font with white color
    margin=dict(l=20, r=50, t=100, b=50),  # Adjust margins to allow more space for the plot
    width=1400,  # Increase figure width
    height=600,  # Increase figure height
...
def branch_college_trend(request):
    if request.method == 'POST':
        ...
        return render(request, 'branch_college_trend.html', {'graph': graph_html, 'branch': branch, 'college_name': college_name})
    else:
